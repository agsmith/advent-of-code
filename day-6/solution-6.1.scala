...
def redistribute(m: Map[Int, Int], k: Int, v: Int): Map[Int,Int] = {
  if(v == 0)
    sortMap(m)
  else {
    val newMap = m.map{ p => if(p._1 == k) (k,p._2 + 1) else p}
    if(k == m.size-1) //wraparound
      redistribute(sortMap(newMap), 0, v-1)
    else
      redistribute(sortMap(newMap), k+1, v-1)
  }
}

def redistributedMap(c: CounterAndMap):CounterAndMap = {
    val k = keyToLargestValue(c.m)
    val v = c.m.get(k).get
    val newM: Map[Int, Int] = sortMap((c.m - k) ++ Map (k -> 0))
    val redistMap = {
      if(k==newM.size-1)
        redistribute(newM, 0, v)
      else
        redistribute(newM, k+1, v)
    }
...
def moves(m: Map[Int, Int], lsss: List[List[Int]], counter: Int): Int = {
  if(lsss.contains(sortMap(m).values.toList)) {
    ...
    val cwl = redistributedMap(CounterAndMap(counter,m))
